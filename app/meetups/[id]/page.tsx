...
export default async function MeetupPage({
  ...
  return (
    <HomePane active="meetups" icon="📍" title={m.title}>
      <Link href="/meetups" className="mb-4 inline-block text-sm text-muted hover:text-bright">
        ← All meetups
      </Link>

      <div className="rounded-2xl border border-line bg-surface p-6 text-white">
        <div className="flex items-center gap-2">
          <h1 className="text-2xl font-bold text-bright">{m.title}</h1>
          {m.isOfficial && (
            <span className="rounded bg-brand-indigo/20 px-1.5 py-0.5 text-xs text-indigo-300">Official</span>
          )}
        </div>
        ...
        <div className="mt-6">
          ...
          {isHost ? (
            <p className="text-sm text-muted">This is your meetup.</p>
          ) : alreadyBooked ? (
            <p className="rounded-lg bg-emerald-400/10 px-4 py-2 text-center font-semibold text-emerald-300">
              Booked ✓
            </p>
          ) : past ? (
            <p className="text-sm text-muted">This meetup has already happened.</p>
          ) : soldOut ? (
            <p className="text-sm text-muted">Sold out.</p>
          ) : (
            <form action={bookMeetupAction}>
              <input type="hidden" name="meetupId" value={m.id} />
              <button className="w-full rounded-lg bg-gradient-to-r from-brand-indigo to-brand-fuchsia px-4 py-3 font-semibold text-white">
